...
def process(item: dict) -> None:
    ...
    Description:
        For the given dictionary instance, copy or compile MIDI files to transfer over to a DB
        that handles filtered and sorted MIDI files
    ...
    mid_files = glob.glob(f"{SOURCE_LOC}\\data\\{item['_id']}\\*.mid") + \
    ...
    # If no MIDI files are present, compile MIDI file from an existing MXL file
        ...
        filename = mxl_files[-1].split("\\")[-1].split(".")[0]    # Get name of the MXL file
        
        filepath = "\\".join(mxl_files[-1].split("\\")[:-1])    # Get file path to the MXL file
        
        # Try to convert and copy data
        try:
            os.system(f"{MUSESCORE} {mxl_files[0]} -o {filepath}\\{filename}.mid")  # Compile the MXL file
            
            # Copy data
            DATA_HANDLE.copy(
                from_path=f"{filepath}\\{filename}.mid",
                index_doc=item
            )
...
if __name__ == "__main__":
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=8) as executor:
        _ = list(tqdm(executor.map(process, SOURCE_INDEX), total=len(SOURCE_INDEX)))
